...
func GetQuotaDatesByTokenModel(c *gin.Context) {
	startTimestamp, _ := strconv.ParseInt(c.Query("start_timestamp"), 10, 64)
	endTimestamp, _ := strconv.ParseInt(c.Query("end_timestamp"), 10, 64)
	tokenID, _ := strconv.Atoi(c.Query("token_id"))
	dates, err := model.GetQuotaDataGroupByTokenModel(startTimestamp, endTimestamp, tokenID)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func GetTokenNames(c *gin.Context) {
	startTimestamp, _ := strconv.ParseInt(c.Query("start_timestamp"), 10, 64)
	endTimestamp, _ := strconv.ParseInt(c.Query("end_timestamp"), 10, 64)
	names, err := model.GetTokenNames(startTimestamp, endTimestamp)
	if err != nil {
		common.ApiError(c, err)
		return
	}
